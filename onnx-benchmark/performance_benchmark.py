...
class Benchmark:
    ...
    def profile(self):
        ...
        if self.args.execution_provider == "iGPU":
            ...
            cache_dir = os.path.join(Path(__file__).parent.resolve(), "cache", os.path.basename(self.args.model_path))
            # Match AMD CNN examples (compile.py / predict.py): stable cache per model stem.
            cache_key = Path(self.args.model_path).stem
    ...
    def report(self):
        if self.args.no_inference == "0":
            ...
            if self.args.execution_provider == "VitisAIEP":
                ...
                cache_key = Path(self.args.model_path).stem
                try:
                    with open(os.path.join(cache_dir, cache_key, "vitisai_ep_report.json"), "r") as json_file:
