...
def evaluate_coco(
    ...
    img_ids: Optional[list] = None,
):
    coco_dt = coco_gt.loadRes(str(detections_path))
    coco_eval = COCOeval(coco_gt, coco_dt, "bbox")
    # Default COCOeval uses all GT val images. If we only inferred a subset
    # (e.g. --eval_max_images), predictions are missing for the rest; those
    # images would count as all false negatives and collapse mAP.
...
def evaluate_on_coco(
    ...
    coco_dataset: str,
    device: str = "cpu",
    num_max_images: Optional[int] = None,
):
    ...
    detections, img_ids_evaluated = evaluate_model(
        session,
        input_name,
        coco,
    ...
    mAP, mAP50, mAP75 = evaluate_coco(
        coco,
        pred_json_save_path,
        PROJECT_DIR / coco_eval_save_path,
        img_ids=img_ids_evaluated,
    )
